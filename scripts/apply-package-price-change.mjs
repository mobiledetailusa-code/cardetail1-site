 ...
 *
 *   node scripts/apply-package-price-change.mjs --percent -20
...
if (!syncOnly && (pctIdx < 0 || !args[pctIdx + 1])) {
  console.error('usage: node scripts/apply-package-price-change.mjs --percent -20 | --sync-only');
  process.exit(1);
}
...
/**
 * Round to $5 above $100 so a quote reads cleanly, to the dollar below that.
 * Per-foot rates stay whole dollars — rounding those to $5 would distort a long
 * boat by far more than the intended percentage.
 */
function repriceValue(value, key) {
  ...
  if (n >= 100) return Math.max(5, Math.round(next / 5) * 5);
...
for (const file of pages) {
  ...
  let drift = 0;
  const after = transformPriceBlocks(before, (ctx) => {

...
(function () {
  ...
  function paymentsHtml(payments) {
    ...
    var rows = payments.map(function (p) {
      return '<tr><td>' + esc(p.date || '') + '</td><td>' + esc(p.method || '') +
        '</td><td class="amt">' + esc(p.amount ? p.amount.display : '') + '</td></tr>';
    }).join('');
    return '<h2>Payments received</h2><div class="tblwrap"><table>' +
      '<thead><tr><th scope="col">Date</th><th scope="col">Method</th><th scope="col" class="amt">Amount</th></tr></thead>' +
  ...
  function render(r) {
    ...
    var totals = '<div class="totals"><div class="tblwrap"><table><tbody>' +
      (vehicles ? '<tr><td>Services subtotal</td><td class="amt">' + esc(fs.serviceLinesTotal ? fs.serviceLinesTotal.display : '') + '</td></tr>' : '') +
      adjustments +
      '<tr class="grand"><td>Approved total</td><td class="amt">' + esc(fs.approvedTotal ? fs.approvedTotal.display : '') + '</td></tr>' +
      '<tr><td>Amount paid</td><td class="amt">' + esc(fs.amountPaid ? fs.amountPaid.display : '') + '</td></tr>' +
    ...
    root.innerHTML =
      ...
      (vehicles ? '<h2>Services</h2>' + vehicles : '') +
      totals +
      paymentsHtml(r.payments) +
      '<p class="foot">Issued by ' + esc(r.business ? r.business.name : '') +
      '. Questions about this receipt? Call or text ' + esc(r.business ? r.business.phone : '') + '.</p>' +
      '</article>';
  }

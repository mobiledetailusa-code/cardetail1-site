// netlify/functions/recent-work-image.js
// Public image-serve endpoint for the Recent Work gallery.
// No auth required — serves only files that were uploaded through recent-work-upload.js.
//
// GET /.netlify/functions/recent-work-image?id=img_<timestamp>_<hex>
// Returns raw image bytes with correct Content-Type and long-lived cache headers.
//
// Security:
//  - ID validated against strict regex before any blob lookup
//  - Companion meta blob must exist with a valid image contentType
//  - Path traversal impossible: IDs contain only digits and hex chars

const ID_RE = /^img_\d+_[0-9a-f]+$/;

const ALLOWED_SERVE_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp']);

async function openImageStore() {
  const { getStore } = await import('@netlify/blobs');
  const siteID = process.env.NETLIFY_SITE_ID;
  const token  = process.env.NETLIFY_AUTH_TOKEN;
  return (siteID && token)
    ? getStore({ name: 'cd1-gallery-images', siteID, token })
    : getStore('cd1-gallery-images');
}

const jsonErr = (status, msg) => ({
  statusCode: status,
  headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' },
  body: JSON.stringify({ ok: false, error: msg }),
});

exports.handler = async (event) => {
  if (event.httpMethod === 'OPTIONS') {
    return { statusCode: 204, headers: { 'Access-Control-Allow-Origin': '*' }, body: '' };
  }
  if (event.httpMethod !== 'GET') return jsonErr(405, 'method_not_allowed');

  const id = ((event.queryStringParameters || {}).id || '').trim();
  if (!ID_RE.test(id)) return jsonErr(400, 'invalid_id');

  try {
    const store = await openImageStore();

    // Companion metadata must exist
    const meta = await store.get(`meta_${id}`, { type: 'json' }).catch(() => null);
    if (!meta || !ALLOWED_SERVE_TYPES.has(meta.contentType)) {
      return jsonErr(404, 'not_found');
    }

    // Fetch image bytes
    const buf = await store.get(id, { type: 'arrayBuffer' }).catch(() => null);
    if (!buf || buf.byteLength === 0) return jsonErr(404, 'not_found');

    return {
      statusCode: 200,
      headers: {
        'Content-Type': meta.contentType,
        // Immutable: IDs are content-addressed — same ID always returns same bytes
        'Cache-Control': 'public, max-age=31536000, immutable',
        'Access-Control-Allow-Origin': '*',
        'X-Content-Type-Options': 'nosniff',
      },
      body: Buffer.from(buf).toString('base64'),
      isBase64Encoded: true,
    };
  } catch (e) {
    return jsonErr(500, 'storage_error');
  }
};

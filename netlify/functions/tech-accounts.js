...
exports.handler = async (event) => {
  ...
  async function listTechs() {
    ...
    for (const b of ((listing && listing.blobs) || [])) {
      ...
      if (t) out.push(projectTechAccountForAdmin(t));
    }
  ...
  if (action === 'list') {
    ...
    const bl = await bookings.list().catch(() => ({ blobs: [] }));
    const counts = {};
    for (const blob of ((bl && bl.blobs) || [])) {
      const bk = await bookings.get(blob.key, { type: 'json' }).catch(() => null);

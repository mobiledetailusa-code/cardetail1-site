...
//
// POST { action, ...params }
//   set_password  { inviteToken, email, newPassword, confirmPassword } → { ok, token }
//   login         { email, password }                                  → { ok, token, techId, name }
//   validate      { token }                                            → { ok, techId, name }
//   logout        { token }                                            → { ok }
...
exports.handler = async (event) => {
  ...
  if (action === 'login') {
    ...
    // Update last login
    result.tech.lastLogin = new Date().toISOString();
    await techStore.setJSON(result.key, result.tech);

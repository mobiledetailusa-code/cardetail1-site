...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { isTechEligibleBooking } = require('../lib/ops-workflow');
    if (!isTechEligibleBooking(booking) && !booking.completionSubmitted) {
    ...
    if (
      ...
      try {
        ...
        if (txn && txn.booking) {
          const expected = Math.max(0, Math.round(Number(patched.bookingVersion) || 0));
          const { ok: nOk, aggregate: nBase } = normalizeAggregate(patched, { allowDraft: false });
          const notifyNext = buildNextAggregate(nOk ? nBase : patched, txn.booking);
          const notifyCommit = await commitBooking({
            bookingId,
            expectedBookingVersion: expected,
            nextAggregate: notifyNext,
            storeOverride: store,
          }).catch(() => null);
          if (notifyCommit && notifyCommit.ok) patched = notifyCommit.booking;
        }
      } catch (e) {

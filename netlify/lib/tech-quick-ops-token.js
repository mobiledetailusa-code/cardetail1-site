 ...
 * Purpose is tech_quick_ops only — never interchangeable with /ops/q, /a?t=, or /pay.
 * GET may mint a session; it must not mutate bookings.
 */
...
function deriveOpaqueToken(bookingId) {
  const digest = crypto.createHmac('sha256', tokenSecret())
    .update(`${PURPOSE_TECH_QUICK_OPS}|${String(bookingId || '').trim()}|${currentEnvBinding()}`)
    .digest();
...
function buildTechOpsUrl(token) {
  return `${siteBase()}/tech/q/${encodeURIComponent(token)}`;
}
...
async function createTechQuickOpsToken({ bookingId, ttlMs = TOKEN_TTL_MS } = {}) {
  ...
  if (
    existing
    && existing.purpose === PURPOSE_TECH_QUICK_OPS
    ...
    return {
      ok: true,
      token,
      techOpsUrl: buildTechOpsUrl(token),
...
async function loadTechQuickOpsToken(rawToken) {
  ...
  if (!envBindingMatches(record) || Date.parse(record.expiresAt) <= Date.now()) {
    return { ok: false, error: 'invalid', statusCode: Date.parse(record.expiresAt) <= Date.now() ? 410 : 400 };
  }
  return { ok: true, bookingId: record.bookingId, expiresAt: record.expiresAt, purpose: record.purpose };
}

async function createTechQuickOpsSession({ bookingId }) {
  const id = String(bookingId || '').trim();
  if (!id) return { ok: false, error: 'booking_required' };
...
async function mintTechOpsUrl(bookingId) {
  try {
    const minted = await createTechQuickOpsToken({ bookingId });
    return minted.ok ? minted.techOpsUrl : '';
  } catch {
    return '';
  }
}

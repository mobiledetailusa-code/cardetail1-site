...
test('guides hub has search, article cards, and FAQ accordion', () => {
  const html = read('blog.html');
  assert.match(html, /id="guide-search"/);
  assert.match(html, /data-guide-card/);
  assert.match(html, /id="faq"/);
  assert.match(html, /<details id="faq-difference">/);
  assert.match(html, /detailing-vs-car-wash\.html/);
  assert.match(html, /how-often-to-detail\.html/);
  assert.match(html, /mobile-detailing-what-to-expect\.html/);
  const types = jsonLdBlocks(html).map((block) => block['@type']);
  assert.ok(types.includes('CollectionPage'));
  assert.ok(types.includes('FAQPage'));
});

test('detailing vs car wash page has comparison table and topic FAQ', () => {

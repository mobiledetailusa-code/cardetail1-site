...
describe('e2e simulate: customer package change → admin approve → customer panel', () => {
  ...
  it('cars: approved total and due appear on customer projection (canonical quote)', () => {
    // Release A: approvable amount from booking-price-catalog, not customer-catalog fixed $290
    ...
    }, { travelCents: 640, basedOnBookingVersion: 1 });
    assert.equal(quoted.ok, true);
    // SUV3 Premium $510 + $10 travel
  ...
  it('addon request: merge + proposed total uses canonical Odor $90 (not portal $149)', () => {
    ...
    assert.equal(quoted.ok, true);
    // SUV3 Full $200 + pet $95 + odor $90 = $255
    ...
    assert.equal(projected.approvedFinalAmount, 255);
    assert.equal(projected.amountDueApproved, 255);
    assert.equal(projected.payLink, '');
  });
});
...
describe('regression: UI approved total vs due after admin regenerate pattern', () => {
  it('old bug: amountDue updated but approvedFinalAmount stale is a conflict', () => {
    const booking = baseBooking({
      ...
      amountDueApproved: 370,   // new Stripe amount
  ...
  it('fixed path: applyPayLinkMoney keeps approved and due aligned', () => {
    ...
    const fixed = { ...booking, ...applyPayLinkMoney(booking, 370, 'https://checkout.stripe.com/new', 'cs') };
    // approved becomes max(old approved, due+paid) = 370
    assert.equal(fixed.approvedFinalAmount, 370);
    assert.equal(fixed.amountDueApproved, 370);

...
test('PRICING: six-package hierarchy and base+ratePerFoot math', () => {
  ...
  assert.equal(prices.full_basic, 765);
  ...
  assert.match(lengthBlock, /maint_light:\s*\{ base: 215, ratePerFoot: 16 \}/);
  assert.match(lengthBlock, /full:\s*\{ base: 340, ratePerFoot: 36 \}/);
});
...
test('DISPLAY: single-price cards and booking CTAs; no funnel', () => {
  ...
  assert.doesNotMatch(page, /From \$899\b|From \$1,?170/);
});

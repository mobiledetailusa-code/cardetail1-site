...
const SMALL_FULL_PLUS_OZONE_CENTS = SMALL_FULL_CENTS + OZONE_CENTS; // 31500
...
describe('Package Stage 1 financial mutations (pre-settlement)', () => {
  ...
  it('1) before-pay upgrade: maint 16000/0/16000 → full 27500/0/27500', async () => {
  ...
  it('2) before-pay downgrade: full 27500/0/27500 → maint 16000/0/16000', async () => {
    const { applyPackageFinancialMutation } = require('../netlify/lib/package-financial-mutation');
  ...
  it('3) add-ons are preserved across a package change (maint+ozone → full+ozone)', async () => {
    ...
    assert.equal(result.ok, true, result.error);
    // Phase 1: Small Car full ($275) + ozone ($40) = $315

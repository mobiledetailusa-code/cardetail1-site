...
describe('Booking confirmation hotfix — frontend contracts', () => {
  for (const page of BOOKING_PAGES) {
    ...
    it(`${page} preserves recoverable slot-failure messaging and form state`, () => {
      ...
      assert.doesNotMatch(
        html.slice(html.indexOf('function showSuccess'), html.indexOf('function showSuccess') + 765),
    ...
    it(`${page} sends form phone on card-status poll`, () => {
      ...
      assert.doesNotMatch(
        html.slice(html.indexOf('async function waitForVerifiedCardSave'), html.indexOf('async function waitForVerifiedCardSave') + 385),
        /draftSessionToken/
      );
    });
  }

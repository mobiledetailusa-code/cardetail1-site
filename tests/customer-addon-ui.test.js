...
describe('Stage 2 post-pay additive carve-out', () => {
  function paidBooking(overrides = {}) {
    return {
      ...
      paymentStatus: 'paid',
      ledger: { approvedCents: 16000, settledCents: 16000, creditedCents: 0, entries: [] },
      quote: { quoteVersion: 1, approvedCents: 16000 },
      service: {
        vehicles: [{
          vehicleId: 'veh_1',
          category: 'cars',
  ...
  it('allows additive ozone after settlement when approvedCents would increase', () => {
    ...
    assert.equal(carve.proposedApprovedCents, 20000);
    assert.equal(carve.currentApprovedCents, 16000);
  });
  ...
  it('duplicate post-pay add is a harmless noop signal', () => {
    const carve = evaluatePostPayAdditiveAddonCarveOut(paidBooking({
      ...
      ledger: { approvedCents: 20000, settledCents: 16000, creditedCents: 0 },
      quote: { quoteVersion: 2, approvedCents: 20000 },
    }), { addonIds: ['ozone'] });
    assert.equal(carve.ok, false);
...
describe('PR4 paid catalog changes remain Admin-reviewed (execution)', () => {
  function paidInvoiceBooking() {
    return {
      ...
      paymentWorkflowStatus: 'payment_succeeded',
      paymentStatus: 'paid',
      ledger: { approvedCents: 16000, settledCents: 16000, creditedCents: 0 },
    };
  }
